...
require_once("Project/plugins/Plugin_Manufacturing_World/Dao/Plugin_Manufacturing_World_ExportDao.php");
require_once("Project/plugins/Plugin_Manufacturing_World/Common/plugin_manufacturing_world_export.php");
...
class  ShowExport extends TForm
{
	public static $Grid1;
	function ShowExport()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowExport","Plugin_Manufacturing_World","form-horizontal row-border",true);
			...
			{
				$form=new TButton();
				$form->set("bn"," เพิ่มข้อมูลประเทศ","","",true,"","");
				...
				$dao_source=new Plugin_SourceDao();
					$o_source=$dao_source->selectAllByDataID($o_manufacturing_world->worldID."_1_2");

				$form=new THidden();
				$form->set("dataID",$o_manufacturing_world->worldID."_1_2","","",true,"","");
				$this->add($form);
			...
			$dao=new Plugin_Manufacturing_World_ExportDao();
				$o=$dao->selectAllByWorldID($worldID);

			ShowExport::$Grid1=new TGridview();
			ShowExport::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowExport::$Grid1->addcontrol($grid);
			...
			ShowExport::$Grid1->View=false;
			ShowExport::$Grid1->Edit=false;
			ShowExport::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowExport::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowExport::$Grid1->gotopage($v[1]);
			ShowExport::$Grid1->changenumpage($v[0]);
		 }
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowExport::$Grid1->getstart();$i<ShowExport::$Grid1->getstop();$i++)
			{
				$o=new plugin_manufacturing_world_export();
				

				$o->exportID=ShowExport::$Grid1->getvalues("exportID",$i);
				
				$o->country=ShowExport::$Grid1->getvalues("country",$i);
				$o->ricevalues=ShowExport::$Grid1->getvalues("ricevalues",$i);

				$o_update=$dao->selectById($o->exportID);
				$o->version=$o_update->version;

				if($o->exportID and $o->ricevalues)
				{
			...
			Refreshs("Plugin_Manufacturing_World/ShowExport&worldID=$worldID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_manufacturing_world_export();
			$o->exportID=ShowExport::$Grid1->getvalues("exportID",$parameter);
			$dao=new Plugin_Manufacturing_World_ExportDao();

			$dao->deletes($o);

			$worldID=$sender->getdata('worldID');
			Refreshs("Plugin_Manufacturing_World/ShowExport&worldID=$worldID","alert","del");
		}
		function frmNew($parameter,$sender)
		{
			...
			$dao=new Plugin_Manufacturing_World_ExportDao();
			$dao->save($o);
			Refreshs("Plugin_Manufacturing_World/ShowExport&worldID=$worldID","alert","save");
		}
		function frmNewAuto($parameter,$sender)
		{
			$array = array("อื่นๆ","กัมพูชา","เมียนมาร์","สหรัฐ","ปากีสถาน","เวียดนาม","อินเดีย","ไทย");
			$worldID=$sender->getdata('worldID');
			$dao=new Plugin_Manufacturing_World_ExportDao();
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Manufacturing_World/ShowExport&worldID=$o->worldID","alert","save");
		...
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowExport::$Grid1->getstart();$i<ShowExport::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowExport::$Grid1->getvalues("cartoonpartIDCheck",$i);
