...
require_once("Project/plugins/Plugin_Manufacturing_World/Dao/Plugin_Manufacturing_World_GenDao.php");
require_once("Project/plugins/Plugin_Manufacturing_World/Common/plugin_manufacturing_world_gen.php");
...
class  ShowGenerate extends TForm
{
	public static $Grid1;
	function ShowGenerate()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowGenerate","Plugin_Manufacturing_World","form-horizontal row-border",true);
			...
			if($o_per[0]->permissAdd=="true")
			{
				...
				$dao_source=new Plugin_SourceDao();
					$o_source=$dao_source->selectAllByDataID($o_manufacturing_world->worldID."_1_1");

				$form=new THidden();
				$form->set("dataID",$o_manufacturing_world->worldID."_1_1","","",true,"","");
				$this->add($form);
			...
			ShowGenerate::$Grid1=new TGridview();
			ShowGenerate::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowGenerate::$Grid1->addcontrol($grid);
			...
			$grid=new TTextBox();
			$grid->set("generate","","","",true,"",false);
			$grid->setAttribute("class","form-control");
			$grid->title='ผลิตข้าว (ล้านตันข้าวสาร)';
			ShowGenerate::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set("consume","","","",true,"",false);
			$grid->setAttribute("class","form-control");
			$grid->title='บริโภคข้าว (ล้านตันข้าวสาร)';
			ShowGenerate::$Grid1->addcontrol($grid);


			ShowGenerate::$Grid1->View=false;
			ShowGenerate::$Grid1->Edit=false;
			ShowGenerate::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowGenerate::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowGenerate::$Grid1->gotopage($v[1]);
			ShowGenerate::$Grid1->changenumpage($v[0]);
		 }
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowGenerate::$Grid1->getstart();$i<ShowGenerate::$Grid1->getstop();$i++)
			{
				$o=new plugin_manufacturing_world_gen();
				

				$o->genID=ShowGenerate::$Grid1->getvalues("genID",$i);
				

				$o->country=ShowGenerate::$Grid1->getvalues("country",$i);
				$o->generate=ShowGenerate::$Grid1->getvalues("generate",$i);
				$o->consume=ShowGenerate::$Grid1->getvalues("consume",$i);

				$o_update=$dao->selectById($o->genID);
				$o->version=$o_update->version;

				if($o->genID and $o->generate and $o->consume)
				{
			...
			Refreshs("Plugin_Manufacturing_World/ShowGenerate&worldID=$worldID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_manufacturing_world_gen();
			$o->genID=ShowGenerate::$Grid1->getvalues("genID",$parameter);
			$dao=new Plugin_Manufacturing_World_GenDao();

			$dao->deletes($o);

			$worldID=$sender->getdata('worldID');
			Refreshs("Plugin_Manufacturing_World/ShowGenerate&worldID=$worldID","alert","del");
		}
		...
		function frmAction($parameter,$sender)
		{
			...
			$o->generate=$sender->getdata('generate_box');
			$o->consume=$sender->getdata('consume_box');
			...
			else
			{
				...
				Refreshs("Plugin_Manufacturing_World/ShowGenerate&worldID=$o->worldID","alert","save");
