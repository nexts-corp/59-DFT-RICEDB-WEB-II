	...
	function ShowNaPrang()
	{
		...
		if($o_per[0]->permissView=="true")
			...
			if($o_per[0]->permissAdd=="true")
			{
				...
				$dao_source=new Plugin_SourceDao();
					$o_source=$dao_source->selectAllByDataID($plantedID."_2_2");

				$form=new THidden();
				$form->set("dataID",$plantedID."_2_2","","",true,"","");
				$this->add($form);
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowNaPrang::$Grid1->getstart();$i<ShowNaPrang::$Grid1->getstop();$i++)
			{
				...
				$o->province=ShowNaPrang::$Grid1->getvalues("province",$i);
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			for($i=3;$i<=count($sheetData);$i++)
			{
				...
				$o->areaFarming=$sheetData[$i]["B"];
				$o->areaHarvest=$sheetData[$i]["C"];
				$o->product=$sheetData[$i]["D"];
				$o->productFarming=$sheetData[$i]["E"];
				$o->productHarvest=$sheetData[$i]["F"];
				$o->percent=$sheetData[$i]["G"];
