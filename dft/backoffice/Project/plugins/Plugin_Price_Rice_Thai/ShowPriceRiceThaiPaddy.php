...
class  ShowPriceRiceThaiPaddy extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddy()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddy","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			if($o_per[0]->permissAdd=="true")
			{
				...
				$form=new TTextBox();
				$form->set('department3_box',"","","",true,"String",true); 
				$form->setAttribute("class","form-control");
				$form->setAttribute("placeholder","ราคาจากสำนักงานเศรษฐกิจฯ");
				$this->add($form);
			...
			ShowPriceRiceThaiPaddy::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddy::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddy::$Grid1->addcontrol($grid);
			...
			$grid=new TTextBox();
			$grid->set("department3","","","",true,"",false);
			$grid->setAttribute("class","form-control");
			$grid->title='ราคาจากสำนักงานเศรษฐกิจฯ';
			ShowPriceRiceThaiPaddy::$Grid1->addcontrol($grid);

			ShowPriceRiceThaiPaddy::$Grid1->View=false;
			ShowPriceRiceThaiPaddy::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddy::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddy::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddy::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddy::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			$o->department3=$sender->getdata('department3_box');
			...
			else
			{
				...
				$o_check_name=$dao->selectAllByThaiIDAndName($o->thaiID,$o->paddyType);
				if(empty($o_check_name))
					$dao->save($o);

				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddy::$Grid1->getstart();$i<ShowPriceRiceThaiPaddy::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy();
				

				$o->paddyID=ShowPriceRiceThaiPaddy::$Grid1->getvalues("paddyID",$i);
				

				$o->department1=ShowPriceRiceThaiPaddy::$Grid1->getvalues("department1",$i);
				$o->department2=ShowPriceRiceThaiPaddy::$Grid1->getvalues("department2",$i);
				$o->department3=ShowPriceRiceThaiPaddy::$Grid1->getvalues("department3",$i);

				$o_update=$dao->selectById($o->paddyID);
				$o->version=$o_update->version;

				if($o->paddyID and $o->department1 and $o->department2 and $o->department3)
				{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_paddy();
			$o->paddyID=ShowPriceRiceThaiPaddy::$Grid1->getvalues("paddyID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy&thaiID=$thaiID","alert","del");
		}
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy&thaiID=$plantedID","alert","save");
		}
