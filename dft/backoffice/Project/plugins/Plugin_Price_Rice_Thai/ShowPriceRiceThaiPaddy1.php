...
class  ShowPriceRiceThaiPaddy1 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddy1()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddy1","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();
				$o=$dao->selectAllByThaiID($thaiID,"1");

			ShowPriceRiceThaiPaddy1::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddy1::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddy1::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiPaddy1::$Grid1->View=false;
			ShowPriceRiceThaiPaddy1::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddy1::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddy1::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddy1::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddy1::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy1&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddy1::$Grid1->getstart();$i<ShowPriceRiceThaiPaddy1::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy();
				

				$o->paddyID=ShowPriceRiceThaiPaddy1::$Grid1->getvalues("paddyID",$i);
				

				$o->department1=ShowPriceRiceThaiPaddy1::$Grid1->getvalues("department1",$i);

				$o_update=$dao->selectById($o->paddyID);
				$o->version=$o_update->version;

				if($o->paddyID and $o->department1)
				{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy1&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_paddy();
			$o->paddyID=ShowPriceRiceThaiPaddy1::$Grid1->getvalues("paddyID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy1&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddy1::$Grid1->getstart();$i<ShowPriceRiceThaiPaddy1::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowPriceRiceThaiPaddy1::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		{
			$paddyID=ShowPriceRiceThaiPaddy1::$Grid1->getvalues("paddyID",$parameter);
			$thaiID=$sender->getdata('thaiID');
			fRefresh("","page","Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData1&paddyID=$paddyID&thaiID=$thaiID");
		}
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy1&thaiID=$thaiID","alert","save");
		}
