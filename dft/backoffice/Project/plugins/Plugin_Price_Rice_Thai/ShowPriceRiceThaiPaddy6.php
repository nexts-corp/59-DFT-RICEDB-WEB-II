...
class  ShowPriceRiceThaiPaddy6 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddy6()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddy6","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();
				$o=$dao->selectAllByThaiID($thaiID,"6");

			ShowPriceRiceThaiPaddy6::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddy6::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddy6::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiPaddy6::$Grid1->View=false;
			ShowPriceRiceThaiPaddy6::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddy6::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddy6::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddy6::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddy6::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy6&thaiID=$o->thaiID","alert","save");
		...
		{
			$o=new plugin_price_rice_thai_paddy();
			$o->paddyID=ShowPriceRiceThaiPaddy6::$Grid1->getvalues("paddyID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy6&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			for($i=ShowPriceRiceThaiPaddy6::$Grid1->getstart();$i<ShowPriceRiceThaiPaddy6::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy();
				
				$o->paddyID=ShowPriceRiceThaiPaddy6::$Grid1->getvalues("paddyID",$i);
				
				$o->cartoonpartIDCheck=ShowPriceRiceThaiPaddy6::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		{
			$paddyID=ShowPriceRiceThaiPaddy6::$Grid1->getvalues("paddyID",$parameter);
			$thaiID=$sender->getdata('thaiID');
			fRefresh("","page","Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData6&paddyID=$paddyID&thaiID=$thaiID");
		}
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy6&thaiID=$thaiID","alert","save");
		}
