...
class  ShowPriceRiceThaiPaddy7 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddy7()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddy7","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();
				$o=$dao->selectAllByThaiID($thaiID,"7");

			ShowPriceRiceThaiPaddy7::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddy7::$Grid1->setview("Grid1",$o);

			for($i=0;$i<count($o);$i++)
			{
				$o[$i]->paddydata="เพิ่มข้อมูลชนิดข้าวสาร";
			}
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddy7::$Grid1->addcontrol($grid);
			
			$grid=new TLabel();
			$grid->set('paddyType',"","","",true,"",false);
			$grid->title='ชนิดข้าวสาร';
			ShowPriceRiceThaiPaddy7::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiPaddy7::$Grid1->View=false;
			ShowPriceRiceThaiPaddy7::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddy7::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddy7::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddy7::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddy7::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				$o_check_name=$dao->selectAllByThaiIDAndName($o->thaiID,$o->paddyType,"7");
				if(empty($o_check_name))
					$dao->save($o);

				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy7&thaiID=$o->thaiID","alert","save");
		...
		{
			$o=new plugin_price_rice_thai_paddy();
			$o->paddyID=ShowPriceRiceThaiPaddy7::$Grid1->getvalues("paddyID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy7&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			for($i=ShowPriceRiceThaiPaddy7::$Grid1->getstart();$i<ShowPriceRiceThaiPaddy7::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy();
				
				$o->paddyID=ShowPriceRiceThaiPaddy7::$Grid1->getvalues("paddyID",$i);
				
				$o->cartoonpartIDCheck=ShowPriceRiceThaiPaddy7::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		{
			$paddyID=ShowPriceRiceThaiPaddy7::$Grid1->getvalues("paddyID",$parameter);
			$thaiID=$sender->getdata('thaiID');
			fRefresh("","page","Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData7&paddyID=$paddyID&thaiID=$thaiID");
		}
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			for($i=3;$i<=count($sheetData);$i++)
			{
				...
				if($o_check_name[0]->paddyID)
				{
					...
					if($o->paddyID)
					{
						...
						$o_data->data9=$sheetData[$i]["J"];
						$o_data->data10=$sheetData[$i]["K"];
						$o_data->data11=$sheetData[$i]["L"];
						$o_data->data12=$sheetData[$i]["M"];
						$o_data->data13=$sheetData[$i]["N"];
						$o_data->data14=$sheetData[$i]["O"];
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy7&thaiID=$thaiID","alert","save");
		}
