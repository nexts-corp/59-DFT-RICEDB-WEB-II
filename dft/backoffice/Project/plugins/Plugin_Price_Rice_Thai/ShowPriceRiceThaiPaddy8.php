...
class  ShowPriceRiceThaiPaddy8 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddy8()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddy8","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();
				$o=$dao->selectAllByThaiID($thaiID,"8");

			ShowPriceRiceThaiPaddy8::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddy8::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddy8::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiPaddy8::$Grid1->View=false;
			ShowPriceRiceThaiPaddy8::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddy8::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddy8::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddy8::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddy8::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				$o_check_name=$dao->selectAllByThaiIDAndName($o->thaiID,$o->paddyType,"8");
				if(empty($o_check_name))
					$dao->save($o);

				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy8&thaiID=$o->thaiID","alert","save");
		...
		{
			$o=new plugin_price_rice_thai_paddy();
			$o->paddyID=ShowPriceRiceThaiPaddy8::$Grid1->getvalues("paddyID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy8&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			$dao=new Plugin_Price_Rice_Thai_PaddyDao();

			for($i=ShowPriceRiceThaiPaddy8::$Grid1->getstart();$i<ShowPriceRiceThaiPaddy8::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy();
				
				$o->paddyID=ShowPriceRiceThaiPaddy8::$Grid1->getvalues("paddyID",$i);
				
				$o->cartoonpartIDCheck=ShowPriceRiceThaiPaddy8::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		{
			$paddyID=ShowPriceRiceThaiPaddy8::$Grid1->getvalues("paddyID",$parameter);
			$thaiID=$sender->getdata('thaiID');
			fRefresh("","page","Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData8&paddyID=$paddyID&thaiID=$thaiID");
		}
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddy8&thaiID=$thaiID","alert","save");
		}
