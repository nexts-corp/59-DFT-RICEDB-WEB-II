...
class  ShowPriceRiceThaiPaddyData4 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddyData4()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddyData4","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			ShowPriceRiceThaiPaddyData4::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddyData4::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);
			
			$grid=new TTextBox();
			$grid->set('data1',"","","",true,"",false);
			$grid->title='เดือน/วันที่';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data2',"","","",true,"",false);
			$grid->title='ท่าข้าว ธ.ก.ส.';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data3',"","","",true,"",false);
			$grid->title='ตลาดกลางพืชไร่';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data4',"","","",true,"",false);
			$grid->title='โรงสีอุดมศิริโชค';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data5',"","","",true,"",false);
			$grid->title='โรงสีกิจทวียโสธร';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data6',"","","",true,"",false);
			$grid->title='โรงสีสหพัฒนา';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data7',"","","",true,"",false);
			$grid->title='โรงสีไฟไทยเจริญวัฒนา';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data8',"","","",true,"",false);
			$grid->title='โรงสีเต็งเฮง';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			$grid=new TTextBox();
			$grid->set('data9',"","","",true,"",false);
			$grid->title='โรงสีสหกรณ์การเกษตร เกษตรวิสัย จำกัด';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData4::$Grid1->addcontrol($grid);

			ShowPriceRiceThaiPaddyData4::$Grid1->View=false;
			ShowPriceRiceThaiPaddyData4::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddyData4::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddyData4::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddyData4::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddyData4::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmNew($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData4&paddyID=$o->paddyID&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddyData4::$Grid1->getstart();$i<ShowPriceRiceThaiPaddyData4::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy_data();
				

				$o->dataID=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("dataID",$i);
				$o->data1=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data1",$i);
				$o->data2=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data2",$i);
				$o->data3=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data3",$i);
				$o->data4=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data4",$i);
				$o->data5=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data5",$i);
				$o->data6=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data6",$i);
				$o->data7=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data7",$i);
				$o->data8=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data8",$i);
				$o->data9=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("data9",$i);
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData4&paddyID=$paddyID&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_paddy_data();
			$o->dataID=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("dataID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_Paddy_DataDao();
			...
			$paddyID=$sender->getdata('paddyID');
			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData4&paddyID=$paddyID&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddyData4::$Grid1->getstart();$i<ShowPriceRiceThaiPaddyData4::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowPriceRiceThaiPaddyData4::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData4&thaiID=$thaiID","alert","save");
		}
