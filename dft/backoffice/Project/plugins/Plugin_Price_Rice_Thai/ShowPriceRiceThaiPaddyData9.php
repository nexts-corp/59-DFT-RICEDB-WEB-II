...
class  ShowPriceRiceThaiPaddyData9 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiPaddyData9()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiPaddyData9","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			ShowPriceRiceThaiPaddyData9::$Grid1=new TGridview();
			ShowPriceRiceThaiPaddyData9::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiPaddyData9::$Grid1->addcontrol($grid);
			...
			$grid->title='ราคา (บาท/ถุง)';
			$grid->setAttribute("class","form-control");
			ShowPriceRiceThaiPaddyData9::$Grid1->addcontrol($grid);

			ShowPriceRiceThaiPaddyData9::$Grid1->View=false;
			ShowPriceRiceThaiPaddyData9::$Grid1->Edit=false;
			ShowPriceRiceThaiPaddyData9::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiPaddyData9::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiPaddyData9::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiPaddyData9::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmNew($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData9&paddyID=$o->paddyID&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddyData9::$Grid1->getstart();$i<ShowPriceRiceThaiPaddyData9::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_paddy_data();
				

				$o->dataID=ShowPriceRiceThaiPaddyData9::$Grid1->getvalues("dataID",$i);
				$o->data1=ShowPriceRiceThaiPaddyData9::$Grid1->getvalues("data1",$i);
				$o->data2=ShowPriceRiceThaiPaddyData9::$Grid1->getvalues("data2",$i);
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData9&paddyID=$paddyID&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_paddy_data();
			$o->dataID=ShowPriceRiceThaiPaddyData9::$Grid1->getvalues("dataID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_Paddy_DataDao();
			...
			$paddyID=$sender->getdata('paddyID');
			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData9&paddyID=$paddyID&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiPaddyData9::$Grid1->getstart();$i<ShowPriceRiceThaiPaddyData9::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowPriceRiceThaiPaddyData9::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiPaddyData9&thaiID=$thaiID","alert","save");
		}
