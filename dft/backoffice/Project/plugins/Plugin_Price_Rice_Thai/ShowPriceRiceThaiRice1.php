...
class  ShowPriceRiceThaiRice1 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiRice1()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiRice1","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			if($o_per[0]->permissAdd=="true")
			{
				...
				$form->setAttribute("class","form-control");
				$form->setAttribute("placeholder","ราคา (บาท)");
				$this->add($form);
			...
			ShowPriceRiceThaiRice1::$Grid1=new TGridview();
			ShowPriceRiceThaiRice1::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiRice1::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiRice1::$Grid1->View=false;
			ShowPriceRiceThaiRice1::$Grid1->Edit=false;
			ShowPriceRiceThaiRice1::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiRice1::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiRice1::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiRice1::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				$o_check_name=$dao->selectAllByThaiIDAndName($o->thaiID,$o->riceType,"2");
				if(empty($o_check_name))
					$dao->save($o);

				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice1&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiRice1::$Grid1->getstart();$i<ShowPriceRiceThaiRice1::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_rice();
				

				$o->riceID=ShowPriceRiceThaiRice1::$Grid1->getvalues("riceID",$i);
				

				$o->department1=ShowPriceRiceThaiRice1::$Grid1->getvalues("department1",$i);

				$o_update=$dao->selectById($o->riceID);
				$o->version=$o_update->version;

				if($o->riceID and $o->department1)
				{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice1&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_rice();
			$o->riceID=ShowPriceRiceThaiRice1::$Grid1->getvalues("riceID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_RiceDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice1&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiRice1::$Grid1->getstart();$i<ShowPriceRiceThaiRice1::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowPriceRiceThaiRice1::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			for($i=3;$i<=count($sheetData);$i++)
			{
				...
				$o_check_name=$dao->selectAllByThaiIDAndName($o->thaiID,$o->riceType,"1");
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice1&thaiID=$thaiID","alert","save");
		}
