...
class  ShowPriceRiceThaiRice2 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiRice2()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiRice2","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			ShowPriceRiceThaiRice2::$Grid1=new TGridview();
			ShowPriceRiceThaiRice2::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiRice2::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiRice2::$Grid1->View=false;
			ShowPriceRiceThaiRice2::$Grid1->Edit=false;
			ShowPriceRiceThaiRice2::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiRice2::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiRice2::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiRice2::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice2&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiRice2::$Grid1->getstart();$i<ShowPriceRiceThaiRice2::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_rice();
				

				$o->riceID=ShowPriceRiceThaiRice2::$Grid1->getvalues("riceID",$i);
				

				$o->department2=ShowPriceRiceThaiRice2::$Grid1->getvalues("department2",$i);

				$o_update=$dao->selectById($o->riceID);
				$o->version=$o_update->version;

				if($o->riceID and $o->department2)
				{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice2&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_rice();
			$o->riceID=ShowPriceRiceThaiRice2::$Grid1->getvalues("riceID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_RiceDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice2&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiRice2::$Grid1->getstart();$i<ShowPriceRiceThaiRice2::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowPriceRiceThaiRice2::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice2&thaiID=$thaiID","alert","save");
		}
