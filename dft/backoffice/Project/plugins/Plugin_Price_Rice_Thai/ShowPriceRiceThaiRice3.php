...
class  ShowPriceRiceThaiRice3 extends TForm
{
	public static $Grid1;
	function ShowPriceRiceThaiRice3()
	{
		...
		if($o_per[0]->permissView=="true")
			$this->Init("ShowPriceRiceThaiRice3","Plugin_Price_Rice_Thai","form-horizontal row-border",true);
			...
			if($o_per[0]->permissAdd=="true")
			{
				...
				$form->setAttribute("class","form-control");
				$form->setAttribute("placeholder","ราคาข้าวถุง (บาท)");
				$this->add($form);
			...
			ShowPriceRiceThaiRice3::$Grid1=new TGridview();
			ShowPriceRiceThaiRice3::$Grid1->setview("Grid1",$o);
			...
			$grid->additem(" ","");
			$grid->title=' ';
			ShowPriceRiceThaiRice3::$Grid1->addcontrol($grid);
			...
			ShowPriceRiceThaiRice3::$Grid1->View=false;
			ShowPriceRiceThaiRice3::$Grid1->Edit=false;
			ShowPriceRiceThaiRice3::$Grid1->Delete=$o_per[0]->permissDel;
			$this->Add(ShowPriceRiceThaiRice3::$Grid1);
		...
		{
			$v=explode(",",$parameter);
			ShowPriceRiceThaiRice3::$Grid1->gotopage($v[1]);
			ShowPriceRiceThaiRice3::$Grid1->changenumpage($v[0]);
		 }
		...
		function frmAction($parameter,$sender)
		{
			...
			else
			{
				...
				Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice3&thaiID=$o->thaiID","alert","save");
		...
		function Grid1_Editing($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiRice3::$Grid1->getstart();$i<ShowPriceRiceThaiRice3::$Grid1->getstop();$i++)
			{
				$o=new plugin_price_rice_thai_rice();
				

				$o->riceID=ShowPriceRiceThaiRice3::$Grid1->getvalues("riceID",$i);
				

				$o->ricebugPrice=ShowPriceRiceThaiRice3::$Grid1->getvalues("ricebugPrice",$i);

				$o_update=$dao->selectById($o->riceID);
				$o->version=$o_update->version;

				if($o->riceID and $o->ricebugPrice)
				{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice3&thaiID=$thaiID","alert","update");

		}
		function Grid1_Deleting($parameter,$sender)
		{
			$o=new plugin_price_rice_thai_rice();
			$o->riceID=ShowPriceRiceThaiRice3::$Grid1->getvalues("riceID",$parameter);
			$dao=new Plugin_Price_Rice_Thai_RiceDao();

			$dao->deletes($o);

			$thaiID=$sender->getdata('thaiID');
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice3&thaiID=$thaiID","alert","del");
		}
		function frmDelAll($parameter,$sender)
		{
			...
			for($i=ShowPriceRiceThaiRice3::$Grid1->getstart();$i<ShowPriceRiceThaiRice3::$Grid1->getstop();$i++)
			{
				...
				$o->cartoonpartIDCheck=ShowPriceRiceThaiRice3::$Grid1->getvalues("cartoonpartIDCheck",$i);
		...
		function frmActionExcel($parameter,$sender)
		{
			...
			Refreshs("Plugin_Price_Rice_Thai/ShowPriceRiceThaiRice3&thaiID=$thaiID","alert","save");
		}
