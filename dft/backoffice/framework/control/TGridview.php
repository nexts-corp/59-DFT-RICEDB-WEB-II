<?php
class TGridview extends TControl
{
	var $Delete=false;
	var $Edit=false;
	var $show_title=true;
	...
	function getDelete($rowindex)
	{
			...
			if($this->Delete=="true")
		{
			...
			//$html=$html. "<td>";
			//$html=$html."<a class='btn' rel='tooltip' title='ลบ' href=\"#\" $ev alt=\"ลบ\"><i class='icon-remove'></i></a>";
			$html=$html."<a class='btn btn-danger btn-xs delete' href='#' $ev><i class='icon-trash bigger-130'></i></a>";
			//$html=$html. "</td>";
		}
		return $html;
	}
	...
	function getEdit($rowindex)
	{
		...
		{
			//$html=$html. "<td>";
			//$html=$html."<a class='btn' rel='tooltip' title='แก้ไข' href=\"#\" $ev alt=\"แก้ไข\"><i class='icon-edit'></i></a>";
			$html=$html."<a class='btn btn-info btn-xs edit' href='#' $ev><i class='icon-pencil bigger-130'></i></a>";
			//$html=$html. "</td>";
		}
		return $html;
	}
		function getSubmit($rowindex)
	{
		...
		{
			//$html=$html. "<td>";
			$html=$html."<a class=\"btn btn-xs\" href=\"#\" $ev alt=\"แสดง\"><i class=\"icon-zoom-in bigger-130\"></i></a>";
			//$html=$html. "</td>";
		}
		return $html;
	}
	...
	function show()
	{
		...
		$html=$html."<table class='table table-hover table-nomargin table-bordered table-colored-header' id=''>";
		...
		for($i=$start;$i<$stop;$i++)
		{
			...
			if($this->Delete== "true" or $this->Edit=="true" or $this->View=="true")
			{
	...
	function Create_Display_page()
	{
		...
		$ev="onchange=\"  javascript:setTimeout('__doPostBack_".$this->formname."(\'".$this->id."_rowperpage"."\',\''+this.options[NumRow.selectedIndex].value+',1'+'\')', 0);\"";
			...
			//$html=$html.="<td>";
			$html=$html."<select Name=\"$NumRN\" id=\"NumRow\" $ev style=\"width: 70px;margin:10px 10px 5px 10px;\">";
