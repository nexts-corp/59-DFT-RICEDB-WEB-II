...
require_once("framework/datasource/connect.php");
require_once("framework/datasource/config.php");
...
class ormdao_data
{
	...
	private $mdb;
	private $clm;
	public function ormdao_data()
	{
		
		global $Config_Data ;
	//	echo $Config_Data["hostname_connect"] ;
		$this->mdb=new ConnectDB;
		$this->mdb->Connect_DB($Config_Data["database_connect"],$Config_Data["hostname_connect"],
		$Config_Data["username_connect"],$Config_Data["password_connect"],$Config_Data["character"]);
	}
   ...
   public function __call($name, $arguments) {
	...
	public function update_object($object)
	{
			...
			$classname=get_class($object);
			$ve=$object->version;
			$sql="update ".$classname." set $fname,version=version+1,whenModified=now() where $pk='$v' and version=$ve";
		//	echo $sql;
